...
export async function GET(request: Request) {
  try {
    ...
    const apiUrl = `${BASE_URL}/${API_KEY}/${coords}/realtime`;
    
    console.log(`代理请求 (实时天气): ${apiUrl}`);
    ...
    console.error('实时天气API代理错误:', error);
    return NextResponse.json(
      { error: '获取实时天气数据时出错', message: error instanceof Error ? error.message : '未知错误' },
      { status: 500 }
    );
  }
} 